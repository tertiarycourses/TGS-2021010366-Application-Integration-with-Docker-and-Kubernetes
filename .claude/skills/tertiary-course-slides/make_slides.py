...
# ============================================================ ADMIN CARD/ICON DESIGNS (WSQ house style)
import os as _os
ICONS=f"{_os.path.dirname(_os.path.abspath(__file__))}/assets/icons"
CENTER=PP_ALIGN.CENTER; MIDDLE=MSO_ANCHOR.MIDDLE
def rrect(s,x,y,w,h,fill,line=LINE):
    sh=s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE,x,y,w,h)
    try: sh.adjustments[0]=0.055
...
def digital_attendance():
    ...
    bullets(s,Inches(0.85),Inches(2.0),Inches(6.3),Inches(4.2),[
        ...
        "A minimum of 75% attendance is required to be eligible for assessment and funding."],size=16,gap=14)
    cx,cy,cw,ch=Inches(7.55),Inches(2.0),Inches(4.8),Inches(3.0)
    rrect(s,cx,cy,cw,ch,LIGHT)
...
def icon_cards(title,kicker,cards,sub=None,cols=3):
    ...
    for k,(icon,t,desc) in enumerate(cards):
        ...
        x=Inches(0.85)+col*(cw+gap); y=y0+row*(ch+Inches(0.28))
        rrect(s,x,y,cw,ch,WHITE)
        d=Inches(0.92); idisc(s,x+(cw-d)//2,y+Inches(0.32),d,icon,BLUE)
...
def grid_cards(title,kicker,cards,cols=3):
    ...
    for k,(icon,t,desc) in enumerate(cards):
        col=k%cols; row=k//cols
        x=Inches(0.85)+col*(cw+gap); y=Inches(1.9)+row*(ch+Inches(0.27))
        rrect(s,x,y,cw,ch,WHITE)
        d=Inches(0.62); idisc(s,x+Inches(0.27),y+Inches(0.27),d,icon,BLUE)
...
def lms_slide():
    s=slide(); head(s,"LMS / TMS","COURSE PORTAL")
    rows=[("ic_lms1","Access course materials","View your course materials, attendance and assessment on the LMS portal."),
          ("ic_lms2","Download the Learner Guide","Download the Learner Guide from the portal for the class activities."),
          ("ic_lms3","Log in","Use your enrolled account to access the materials and assessment.")]
    ry,gap,d=Inches(2.05),Inches(1.0),Inches(0.74)
    for icon,lab,desc in rows:
        idisc(s,Inches(0.85),ry,d,icon,BLUE)
        txt(s,Inches(0.85)+d+Inches(0.32),ry-Inches(0.03),Inches(10.0),d+Inches(0.06),
            [[(lab,16,INK,True)],[(desc,14,GREY,False)]],anchor=MIDDLE,space=2)
        ry+=gap
    rrect(s,Inches(0.85),Inches(5.2),Inches(11.65),Inches(0.7),LIGHT)
    txt(s,Inches(1.2),Inches(5.2),Inches(11),Inches(0.7),
        [[("LMS Portal:   ",15,BLUE,True),("https://lms-tms.tertiaryinfotech.com",15,INK,False)]],anchor=MIDDLE)
    footer(s)

def lesson_plan_cards(days,timing):
    s=slide(); head(s,"Lesson Plan — 3 Days, 8 hours/day","SCHEDULE")
    gap=Inches(0.33); cw=(Inches(11.65)-gap*2)//3; cy,ch=Inches(1.92),Inches(3.55)
    for k,(num,lines) in enumerate(days):
        ...
        para=[]
        for t,bold in lines: para.append([(t,12.5,INK if bold else GREY,bold)])
        txt(s,x+Inches(0.32),cy+Inches(1.35),cw-Inches(0.64),Inches(2.0),para,space=6)
    rrect(s,Inches(0.85),Inches(5.7),Inches(11.65),Inches(0.62),LIGHT)
    txt(s,Inches(1.2),Inches(5.7),Inches(11),Inches(0.62),[[("Daily timing:   ",14,BLUE,True),(timing,14,INK,False)]],anchor=MIDDLE)
    footer(s)

def learning_outcomes(los):
    s=slide(); head(s,"Learning Outcomes","WHAT YOU'LL ACHIEVE")
    ry,gap,d=Inches(1.95),Inches(0.92),Inches(0.64)
    for i,lo in enumerate(los,1):
        ov=oval(s,Inches(0.85),ry,d,d,BLUE); tf=ov.text_frame; p=tf.paragraphs[0]; p.alignment=CENTER
        r=p.add_run(); r.text=str(i); r.font.size=Pt(20); r.font.bold=True; r.font.color.rgb=WHITE; r.font.name=FONT
        txt(s,Inches(0.85)+d+Inches(0.34),ry,Inches(10.6),d,
            [[("LO"+str(i)+"   ",16,BLUE,True),(lo,16,INK,False)]],anchor=MIDDLE)
        ry+=gap
    footer(s)

def assessment_twocard():
    s=slide(); head(s,"Assessment","FINAL ASSESSMENT")
    cy,ch=Inches(1.95),Inches(4.0); lw=Inches(5.75)
    lx=Inches(0.85); rx=lx+lw+Inches(0.3)
    rrect(s,lx,cy,lw,ch,WHITE)
    d=Inches(0.72); idisc(s,lx+Inches(0.34),cy+Inches(0.34),d,"ic_clip",BLUE)
    ...
    bullets(s,lx+Inches(0.34),cy+Inches(1.4),lw-Inches(0.68),Inches(2.4),[
        ...
        "Mini capstone project presented on Day 3","Appeal process available if required"],size=13,gap=10)
    rrect(s,rx,cy,lw,ch,BLUE,line=None)
    oval(s,rx+Inches(0.34),cy+Inches(0.34),d,d,RGBColor(0x0B,0x4F,0xC0))
    if _os.path.exists(f"{ICONS}/ic_award.png"):
        pad=int(d*0.27); s.shapes.add_picture(f"{ICONS}/ic_award.png",rx+Inches(0.34)+pad,cy+Inches(0.34)+pad,width=d-2*pad,height=d-2*pad)
    ...
    txt(s,rx+Inches(0.34),cy+Inches(2.75),lw-Inches(0.68),Inches(1.3),
        [[("Assessed as Competent",15,WHITE,True)],[("Pass both the Written Assessment and Practical Performance.",13,LT,False)]],space=6)
    footer(s)
...
def assessment_flow():
    ...
    steps=[("ic_qr","STEP 1","TRAQOM","Scan the TRAQOM QR code on the LMS and complete the survey."),
           ...
           ("ic_clip","STEP 3","Assessment","Complete the Written Assessment & Practical Performance."),
    ...
    txt(s,Inches(0.85),Inches(5.5),Inches(11.6),Inches(0.34),
        [[("TIP   ",13,TEAL,True),("Courseware & the assessment are on the LMS:  https://lms-tms.tertiaryinfotech.com",13,GREY,False)]])
    footer(s)

def cert_traqom():
    s=slide(); head(s,"Certification & TRAQOM Survey","BEFORE YOU GO")
    cy,ch=Inches(2.0),Inches(3.6); lw=Inches(5.75)
    lx=Inches(0.85); rx=lx+lw+Inches(0.3)
    rrect(s,lx,cy,lw,ch,WHITE)
    ...
    txt(s,lx+Inches(0.36),cy+Inches(1.5),lw-Inches(0.72),Inches(1.6),
        [[("Complete the certification & TRAQOM survey to receive your certificate.",14.5,INK,False)],
         [("lms-tms.tertiaryinfotech.com",14.5,BLUE,True)]],space=8)
    rrect(s,rx,cy,lw,ch,BLUE,line=None)
    oval(s,rx+Inches(0.36),cy+Inches(0.36),d,d,RGBColor(0x0B,0x4F,0xC0))
    if _os.path.exists(f"{ICONS}/ic_check.png"):
        pad=int(d*0.27); s.shapes.add_picture(f"{ICONS}/ic_check.png",rx+Inches(0.36)+pad,cy+Inches(0.36)+pad,width=d-2*pad,height=d-2*pad)
...
digital_attendance()
about_trainer_blank()
about_trainer_filled("Dr. Alfred Ang","AI Trainer & Engineer",[
...
icon_cards("Let's Know Each Other","ICE-BREAKER",[
 ...
 ("ic_target","Your goal","What you want to automate after this course.")],
...
grid_cards("Ground Rules","HOUSE RULES",[
 ...
 ("ic_gr3","Mutual respect","Agree to disagree; one conversation at a time."),
 ("ic_gr4","Be punctual","Return from breaks on time."),
 ("ic_gr6","Step out quietly","For calls or toilet breaks."),
 ("ic_gr5","75% attendance","Required for funding eligibility.")])
lms_slide()
lesson_plan_cards([
 ("1",[("Topic 1 — n8n basics",True),("·  Act 1, 2, 3a, 3b",False),("Topic 2 — AI Agents",True),("·  Act 4a, 4b",False)]),
 ("2",[("Topic 3 — Webhooks",True),("·  Act 5",False),("Topic 4 — APIs & HTTP",True),("·  Act 6",False),("Topic 5 — RAG",True),("·  Act 7, 7b",False)]),
 ("3",[("Topic 6 — Security & Guardrails",True),("·  Act 8",False),("Topic 7 — Mini Capstone",True),("·  + presentations",False)])],
 "9:30am – 6:30pm   ·   1-hour lunch   ·   short tea breaks within each day")
learning_outcomes([
 "Build automated workflows in n8n using triggers, actions, nodes and flows.",
 "Design AI agents with LLMs, memory and tools, triggered from chat / Telegram.",
 "Apply Retrieval-Augmented Generation (RAG) to ground agents in your documents.",
 "Integrate external systems via webhooks, APIs and HTTP requests.",
 "Apply human-in-the-loop and guardrails to secure agentic automations."])
assessment_twocard()
grid_cards("Briefing for Assessment","ASSESSMENT RULES",[
 ("ic_br1","Clear your space","Place phones and other materials under the table or on the floor."),
 ("ic_br2","No recording","No photos or recording of assessment scripts."),
 ("ic_br3","Work individually","No discussion during the assessment."),
 ("ic_br4","Use a pen","Black or blue pen for hard-copy assessments."),
 ("ic_br5","No correction tape","No liquid paper or correction fluid."),
 ("ic_br6","Time’s up","Scripts are collected when time is up.")])
